...
function LibraryView() {
  ...
  return <div className="page-stack library-page"><section className="library-heading"><div><p className="eyebrow">参考书架</p><h1>从案例走向文本</h1><p>陆致极体系按书目与版本关系整理，另保留古籍与近现代参照。书目用于查证和比较，不会自动改写案例结论。</p></div><div><strong>{REFERENCE_BOOKS.length}</strong><span>核验书目 · 陆致极体系 {luBooks.length} 项</span></div></section>
    ...
    <section className="library-bibliography-note"><CheckCircle size={22} weight="fill" /><div><strong>需购入实体书后阅读</strong><p>本站只整理书目、版本关系与学习顺序，不提供电子书正文。15 项包含独著、合著和版本参考，不等同于 15 部内容互不重复的独立作品。</p></div></section>
